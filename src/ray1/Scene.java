...
//		return accelStruct.intersect(new IntersectionRecord(), ray, true);
	...
	private boolean intersect(IntersectionRecord outRecord, Ray rayIn, boolean anyIntersection) {
		...
		IntersectionRecord checkRecord = new IntersectionRecord();
		for(Surface i: this.getSurfaces())
		{
			if(i.intersect(checkRecord, rayIn)) {
				if(checkRecord.t < outRecord.t && anyIntersection) {
					return true;
				}else if(checkRecord.t < min_t) {
					min_t = checkRecord.t;
					outRecord.set(checkRecord);
					ret = true;
					outRecord.surface = i;
				}
			}
