...
public class PhongBRDF extends BRDF {
	...
	{	
		...
		Vector3 specular = specularColor.clone().mul((float)Math.pow(Math.max(surfaceNormal.dot(h), 0), exponent));
		BRDFValue.set(getDiffuseReflectance(texCoords).clone().div((float)Math.PI).add(specular));
	}
